...
public class View {
	...
	/**
	 * rendering the total sum table Hashtable contains the calculation results from
	 * Data Model
	 */
	...
	public void addTotal(Hashtable<String, Double> total) {
		...
		Object[][] data = new Object[][] { { "Summe", decFormat2.format(total.get("brutto")) + "€" },
				{ "-Rabatt " + decFormat1.format(total.get("discount_rel")) + "%",
						decFormat2.format(total.get("discount_rel_abs")) + "€" },
				{ "-Rabatt absolut", decFormat2.format(total.get("discount_abs")) + "€" },
				{ "Endbetrag", decFormat2.format(total.get("netto")) + "€" } };
