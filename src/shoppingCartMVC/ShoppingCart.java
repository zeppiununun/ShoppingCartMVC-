...
public class ShoppingCart {
	...
	/**
	 * searches for a Product object in the shopping cart @return=-1, if not found
	 * otherwise returns its index in the itemList
	 */

	int searchItem(Product p) {
		for (LineItem q : itemList) { // the product is there
	...
	/**
	 * adds a neu product to the shopping cart only one item
	 */
	...
	/**
	 * Either removes a product completely from the shopping cart(count= its
	 * quantity) or reduces its quantity by count
	 */
	 ...
	 * 
	 * @return Hashtable key: "brutto" the sum without discounting key:
	 *         "discount_rel_abs" % discount in Euro key: "discount_abs" absolute
	 *         discount key: "discount" % discount + absolute discount key: "netto"
	 *         discounted sum key: "cart_size" a number of items in the shopping
	 *         cart key: "discount_rel_abs" % discount in %
	 */
	...
	// a number of positions in the shopping cart
